...
getLDpairs <- function(eqtls, genotype, minFDR=0.05, maxP=NULL, minR=0.8, genoOpt=getOptions(),
	...
	if(nrow(eqtls) > 0){
		...
		## get list of R-sq between peak SNP for each gene and all other SNPs
...
.submitLDpairs <- function(selGene, eqtls, geno, maxP, minR, progressBar){
	...
	while(nrow(eqtls) > 0){
		eqtls <- subset(eqtls, snps %in% c(as.character(eqtls$snps[1]), rownames(geno)))
		proxies <- data.frame(snp1=character(), snp2=character(), 
				Rsquared=numeric(),	pvalue=numeric(), stringsAsFactors=FALSE)
		...
		if(nrow(eqtls) == 1){
			...
			ret <- .computeLD(geno = geno, eqtls = eqtls, selGene = selGene, 
					maxP = maxP, Rsquared = Rsquared, minR = minR)		
		}
		eqtls <- subset(eqtls[-1,], snps %in% ret$groups$snps)
		ans$groups <- rbind(ans$groups, ret$groups)
		ans$proxies <- rbind(ans$proxies, ret$proxies)
	}
	if(!sge.getOption("sge.use.cluster")){
...
.computeLD <- function(eqtls, geno, selGene, maxP, minR){
	proxies <- proxies <- data.frame(snp1=character(), 
			snp2=character(), Rsquared=numeric(), pval=numeric(), 
			stringsAsFactors=FALSE)
	selected <- eqtls[1,]
	selected$others <- NA
	...
	if(nrow(eqtls) > 1){
		...
		if(nrow(snpMat)){
			...
			command <- paste("python", system.file(file.path("exec", "cubex-cl.py"), 
							package="mePipe"), tmp, ">", outFile)
			system(command)
			
			## parse CubeX output
			cubexOut <- XML::xmlTreeParse(outFile)
			cubexOut <- lapply(XML::xmlChildren(cubexOut$doc$children[["dataset"]]), 
					XML::xmlToList)
			...
			for(i in 1:length(cubexOut)){
				...
				if(is.null(solution)){
					rsq <- c(rsq, NA)
					warning("No solution found for SNP pair ", eqtls$snps[1], ", ", 
							eqtls$snps[i+1])
				} else {
					rsq <- c(rsq, thisR)
				}
			...
			if(nrow(selectedProxies) >= 1){
				...
				selected$Rsquared <- paste(sprintf("%.03f", selectedProxies$Rsquared), 
						collapse=",")
			} 
